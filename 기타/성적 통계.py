...
for i in range(k):
    classScores = list(map(int, input().split()))
    classScores.remove(classScores[0])
    classScores.sort(reverse=True)

    gap = 0
    current = classScores[0]
    for j, v in enumerate(classScores):
        if j == 0:
            continue
        if gap < current - v:
            gap = current - v
        current = v
    print("Class {0}\nMax {1}, Min {2}, Largest gap {3}".format(i+1, max(classScores), min(classScores), gap))
