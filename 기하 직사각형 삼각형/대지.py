...
n = int(input())
ock = [tuple(map(int,(input().split()))) for _ in range(n)]

if n == 1:
    print(0)
else:
    # 직사각형의 가로 세로 구하기
    max_x = max([ock[i][0] for i in range(n)])
    min_x = min([ock[i][0] for i in range(n)])
    max_y = max([ock[i][1] for i in range(n)])
    min_y = min([ock[i][1] for i in range(n)])
